...
import aiohmi.ipmi.fru as fru
import aiohmi.ipmi.oem.generic as genericoem
from aiohmi.ipmi.oem.lookup import get_oem_handler
...
try:
    ...
    session = None
try:
    from aiohmi.ipmi.private import localsession
except ImportError:
    localsession = None
...
def select_simplesession():
    global session
    import aiohmi.ipmi.private.simplesession as session
        ...
        if os.path.exists(directory) and not os.path.isdir(directory):
            raise exc.InvalidParameterValue(
                'Not allowed to overwrite existing file: {0}'.format(
                    directory))
        await self.oem_init()
        async for lic in self._oem.save_licenses(directory):
            yield lic
